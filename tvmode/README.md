...
Wake behavior handles the S95B's two standby depths. If REST reports `PowerState: "on"`, the TV is treated as genuinely awake and `assumeInputWhenOn` may skip navigation. If REST is reachable but `PowerState` is `standby`, missing, or ambiguous, `tvmode` treats that as fast standby: it connects to the tokenized websocket, sends `KEY_POWER`, polls REST until `PowerState: "on"` for up to about 15 seconds, then always runs KEY_SOURCE navigation. If REST is unreachable, `tvmode` treats that as deep standby/off: it sends WoL, waits for port `8002`, then runs full KEY_SOURCE navigation. `tvmode couch --force-input` overrides the on-state skip and sends KEY_SOURCE navigation regardless.
