...
`tvmode desk` makes the desk display primary, restores windows minimized by the last `couch` run, and switches audio back to the desk device. It leaves HDR unchanged and does not power off or otherwise touch the TV.
...
| `hdr` | `on` enables HDR on the couch display during `couch`; `off` skips HDR changes. Defaults to `on`. |
...
HDR is enable-only and targets only the display matched by `couchDisplayMatch`. During `couch`, Windows 11 state is read with `GET_ADVANCED_COLOR_INFO_2`, enabled with `SET_HDR_STATE` when needed, and re-read until `activeColorMode` confirms HDR; legacy advanced-color APIs are fallback only. After a cold attach or mode correction, HDR waits an additional three seconds following the primary-display change. `desk` neither queries nor changes HDR. HDR errors are logged as warnings and do not stop the rest of the transition.
...
See the [changelog](CHANGELOG.md) and [implementation notes](NOTES.md) for release history, Samsung API details, diagnostics, and the tested S95B input behavior.
